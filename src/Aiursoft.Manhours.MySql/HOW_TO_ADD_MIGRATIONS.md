...
1. Navigate to the MySQL project directory:
   ```bash
   cd ./src/Aiursoft.Manhours.MySql/
   ```
...
```bash
dotnet ef migrations remove --context "MySqlContext" -s ../Aiursoft.Manhours/Aiursoft.Manhours.csproj
```

## After Creating Migrations

After creating migrations for MySQL, remember to:
1. Create the corresponding migration for SQLite (see `../Aiursoft.Manhours.Sqlite/HOW_TO_ADD_MIGRATIONS.md`)
